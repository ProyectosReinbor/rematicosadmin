...
function loadTemplate(templateId: string): Record<string, unknown> | null {
  const templateFile = path.join(TEMPLATES_DIR, `template_${templateId}.json`);
...
router.post("/generar", async (req: Request, res: Response) => {
  try {
    ...
    const imageValidation = validateImage(imageBase64, mimeType);
...
router.get("/template-config/:id", (req: Request, res: Response) => {
